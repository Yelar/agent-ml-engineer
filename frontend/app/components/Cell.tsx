...
const markdownComponents: Components = {
  ...
  code: ({ children, ...props }) => (
    <code
      {...props}
      className="rounded-md bg-slate-900/60 px-1.5 py-0.5 text-[0.85rem]"
    >
      {children}
    </code>
  ),
...
export default function Cell({ event }: CellProps) {
  ...
  if (event.type === "code" && isRecord(event.payload)) {
    ...
    return (
      <article className="animate-cell-enter space-y-4 rounded-3xl border border-slate-800/80 bg-slate-900/80 p-6 shadow-[0_32px_90px_-50px_rgba(15,23,42,0.9)]">
        ...
        {code ? (
          <pre className="overflow-x-auto rounded-2xl border border-slate-800 bg-slate-950/80 p-4 text-xs font-mono leading-6 text-slate-100 shadow-inner wrap-break-word whitespace-pre-wrap">
            {code}
          </pre>
        ) : null}
        {output ? (
          <section className="space-y-2">
            ...
            <pre className="overflow-x-auto rounded-2xl border border-slate-800 bg-slate-900/80 p-4 text-sm leading-6 text-slate-100 wrap-break-word whitespace-pre-wrap max-h-96 overflow-y-auto">
              {output}
            </pre>
          </section>
        ) : null}
  ...
  if (
    ...
    typeof event.payload.image === "string"
  ) {
    return (
        ...
        <dl className="grid grid-cols-1 gap-2 text-sm text-slate-200 md:grid-cols-2">
          {Object.entries(event.payload).map(([key, value]) => (
            ...
            >
              ...
              <dd className="wrap-break-word text-slate-100">
                {String(value ?? "")}
              </dd>
            </div>
          ))}
        ...
        {items.length === 0 ? (
          ...
          <ul className="space-y-2 text-sm text-slate-100">
            {items.map((item, itemIndex) => {
              const name =
                typeof item.name === "string" ? item.name : "artifact";
  ...
  if (event.type === "status" && isRecord(event.payload)) {
    const stage =
      typeof event.payload.stage === "string" ? event.payload.stage : "pending";
    const message =
