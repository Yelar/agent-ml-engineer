...
export default function Cell({ event }: CellProps) {
  ...
  if (event.type === 'code' && isRecord(event.payload)) {
    ...
    return (
      <article className="animate-cell-enter space-y-4 rounded-xl border border-border bg-card p-5 shadow-sm">
        <header className="flex items-center justify-between text-xs uppercase tracking-wide text-muted-foreground">
          ...
          {timestamp ? <span className="text-muted-foreground/60">{timestamp}</span> : null}
        </header>
        {code ? (
          <pre className="rounded-lg bg-foreground/90 p-4 text-xs font-mono leading-6 text-background shadow-inner">
            {code}
          </pre>
        ) : null}
        {output ? (
          <section className="space-y-2">
            <div className="text-xs uppercase tracking-wide text-emerald-600 dark:text-emerald-400">Out&nbsp;[{stepLabel}]</div>
            <pre className="rounded-lg bg-muted p-4 text-sm leading-6 text-foreground">{output}</pre>
          </section>
        ) : null}
        {error ? (
          <section className="space-y-2">
            <div className="text-xs uppercase tracking-wide text-red-600 dark:text-red-400">Error</div>
            <pre className="rounded-lg bg-destructive/10 p-4 text-sm leading-6 text-destructive">{error}</pre>
          </section>
        ) : null}
      </article>
    );
  ...
  if (event.type === 'plot' && isRecord(event.payload) && typeof event.payload.image === 'string') {
    return (
      <article className="animate-cell-enter space-y-3 rounded-xl border border-border bg-card p-5 shadow-sm">
        <header className="flex items-center justify-between text-xs uppercase tracking-wide text-muted-foreground">
          <span className="font-semibold text-foreground">Figure&nbsp;{stepLabel}</span>
          {timestamp ? <span className="text-muted-foreground/60">{timestamp}</span> : null}
        </header>
        <img
          src={`data:image/png;base64,${event.payload.image}`}
          alt={`Notebook figure ${stepLabel}`}
          className="max-h-[420px] w-full rounded-lg border border-border bg-muted object-contain p-2"
        />
      </article>
    );
  }
  ...
  if (event.type === 'metadata' && isRecord(event.payload)) {
    return (
      <article className="animate-cell-enter space-y-3 rounded-xl border border-border bg-card p-5 shadow-sm">
        <header className="flex items-center justify-between text-xs uppercase tracking-wide text-muted-foreground">
          <span className="font-semibold text-foreground">Run Metadata</span>
          {timestamp ? <span className="text-muted-foreground/60">{timestamp}</span> : null}
        </header>
  ...
  if (event.type === 'artifacts' && isRecord(event.payload)) {
    ...
    return (
      <article className="animate-cell-enter space-y-3 rounded-xl border border-border bg-card p-5 shadow-sm">
        <header className="flex items-center justify-between text-xs uppercase tracking-wide text-muted-foreground">
          <span className="font-semibold text-foreground">Artifacts</span>
          {timestamp ? <span className="text-muted-foreground/60">{timestamp}</span> : null}
        </header>
        {items.length === 0 ? (
            ...
            {items.map((item, itemIndex) => {
              ...
              return (
                ...
                >
                  ...
                  {url ? (
                    <a
                      className="underline decoration-primary/40 decoration-dotted underline-offset-2 hover:text-primary/80"
                      href={url}
                      download
                      target="_blank"
                      rel="noreferrer"
  ...
  if (event.type === 'status' && isRecord(event.payload)) {
    ...
    return (
      <article className="animate-cell-enter space-y-2 rounded-xl border border-dashed border-border bg-muted p-4 text-sm text-muted-foreground">
