...
export default function App() {
  ...
  const handleFilesSelected = useCallback(
    async (files: FileList) => {
      ...
      try {
        // For websocket_server.py: send one file at a time with field name 'csv'
        // For server.py: send all files at once with field name 'files'
        // Try websocket_server.py format first (single file)
        if (selected.length === 1) {
          const formData = new FormData();
          formData.append('csv', selected[0]);
          ...
          // Multiple files - try server.py format
          const formData = new FormData();
